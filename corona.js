...
window.onload = function () {
  ...
  function covid19() {
    ...
    fetch(url, requestOptions)
      ...
      .then((data) => {
        ...
        if( totalIndia[lengthIndia-1].lastupdatedtime <data.statewise[0].lastupdatedtime){
        ...
        document.getElementById("deaths").innerHTML = data.statewise[0].deaths;
         ...
         document.getElementById("percentage").innerHTML =
          ...
          ((data.statewise[0].recovered / data.statewise[0].active) * 100).toFixed(
           2
         ) + "%";
         ...
         let last_week_death;
             ...
             current_week_recovery =data.cases_time_series[case_length-1].totalrecovered-data.cases_time_series[case_length-8].totalrecovered;
             last_week_recovery =data.cases_time_series[case_length-9].totalrecovered-data.cases_time_series[case_length-17].totalrecovered;
             current_week_death =data.cases_time_series[case_length-1].totaldeceased-data.cases_time_series[case_length-8].totaldeceased;
             last_week_death =data.cases_time_series[case_length-9].totaldeceased-data.cases_time_series[case_length-17].totaldeceased;
        ...
        // console.log(current_week_recovery);
             ...
             (((current_week_recovery- last_week_recovery) /  last_week_recovery) * 100).toFixed(
              2
            ) + "%";
            document.getElementById("deathtrend").innerHTML =
             (((current_week_death- last_week_death) /  last_week_death) * 100).toFixed(
              2
            ) + "%";
        ...
        let stateName = [];
           ...
           // console.log(tempArray);
               ...
               x.innerHTML+='<div class=\"col-3\"><h4>'+stateName[indexNo]+'</h4></div><div class=\"col-2\"><h2>'+stateActive[indexNo]+'</h2></div><div class=\"col-2\"><h2>'+stateConfirmed[indexNo]+'</h2></div><div class=\"col-2\"><h2>'+stateDeath[indexNo]+'</h2></div><div class=\"col-2\"><h2>'+stateRecovered[indexNo]+'</h2></div>';
               }

 
   let rateRecovery =stateRecovered.map(function(n, i) { return n *100/ stateConfirmed[i]; });
   
   console.log(rateRecovery);
   let tempArray1 = [...rateRecovery];
   tempArray1.sort((a, b) => b-a);
    $j=0;
    document.getElementById("trend_rate_recover").innerHTML='<div class=\"row\"><div class=\"col-7\"><h2>State</h2></div><div class=\"col-5\"><h2>Recovery Rate</h2></div></div>';
   for($i=0; $i<5+$j; $i++)
   {  if(tempArray1[$i]<100){
 ...
 document.getElementById("trend_rate_recover").innerHTML+='<div class=\"row\"><div class=\"col-7\"><h4>'+stateName[indexNo1]+'</h4></div><div class=\"col-5\"><h2>'+(rateRecovery[indexNo1]).toFixed(2)+'</h2></div></div>';
   }
   ...
   let rateDeath =stateDeath.map(function(n, i) { return n *100/ stateConfirmed[i]; });
   document.getElementById("trend_rate_death").innerHTML='<div class=\"row\"><div class=\"col-7\"><h2>State</h2></div><div class=\"col-5\"><h2>Death Rate</h2></div></div>';
 ...
 document.getElementById("trend_rate_death").innerHTML+='<div class=\"row\"><div class=\"col-7\"><h4>'+stateName[indexNo2]+'</h4></div><div class=\"col-5\"><h2>'+(rateDeath[indexNo2]).toFixed(2)+'</h2></div></div>';
        ...
        for ($i = 5; $i >0; $i--) {
              ...
              myFunction(data.tested[$len - $i].positivecasesfromsamplesreported, 'A', "case_trends");
            }
